...
export const About = ({ className }: AboutProps) => {
  return (
    <section id="about" className={cn('w-full relative overflow-hidden bg-gradient-to-b from-[#014B9F] to-[#003366]', className)}>
      ...
      <div className="absolute top-0 left-0 right-0 h-[100px] sm:h-[150px] bg-white transform -skew-y-3 -translate-y-1/2" />
      <div className="absolute bottom-0 left-0 right-0 h-[100px] sm:h-[150px] bg-white transform -skew-y-3 translate-y-1/2" />
      <div className="relative max-w-[1920px] mx-auto px-4 sm:px-6 md:px-12 py-16 sm:py-24 md:py-32 text-white">
        <motion.div
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true }}
          variants={containerVariants}
          className="flex flex-col items-center gap-8 sm:gap-16 p-4 sm:p-6 md:p-8 lg:p-12"
        >
          <motion.div variants={itemVariants} className="text-center max-w-3xl">
            <motion.h2
              {...fadeInLeft}
              className="text-2xl sm:text-[32px] md:text-[48px] lg:text-[60px] font-bold mb-4 sm:mb-6"
            >
              О компании
            </motion.h2>
          ...
          <motion.div variants={itemVariants} className="w-full max-w-6xl">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8">
              <div className="space-y-4 sm:space-y-8">
                <div className="bg-white/5 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-8 hover:bg-white/10 transition-all duration-300">
                  <div className="space-y-4 sm:space-y-6">
                    ...
                    <div className="grid grid-cols-1 xs:grid-cols-2 gap-2 sm:gap-4">
                      {industries.map((industry, index) => (
                        <div key={index} className="flex items-center gap-2 sm:gap-3 bg-white/5 rounded-lg p-2 sm:p-3">
                          <industry.icon className="w-5 h-5 sm:w-6 sm:h-6 text-blue-400" />
                          <span className="text-sm sm:text-base text-gray-200">{industry.title}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="bg-white/5 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-8 hover:bg-white/10 transition-all duration-300">
                  <div className="space-y-4 sm:space-y-6">
                    ...
                    <div className="flex items-center gap-3 sm:gap-4 text-blue-400">
                      <Clock className="w-5 h-5 sm:w-6 sm:h-6" />
                      <span className="text-sm sm:text-base text-gray-200">Быстрая доставка</span>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="bg-white/5 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-8 hover:bg-white/10 transition-all duration-300">
                  <div className="space-y-4 sm:space-y-6">
                    ...
                    <div className="flex items-center gap-3 sm:gap-4 text-blue-400">
                      ...
                      <span className="text-sm sm:text-base text-gray-200">Современные технологии</span>
                    </div>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-2 sm:gap-4">
                  <div className="bg-white/5 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 hover:bg-white/10 transition-all duration-300">
                    <div className="flex flex-col items-center text-center gap-2 sm:gap-3">
                      <Target className="w-6 h-6 sm:w-8 sm:h-8 text-blue-400" />
                      <span className="text-sm sm:text-base text-gray-200">Качество</span>
                    </div>
                  </div>
                  <div className="bg-white/5 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 hover:bg-white/10 transition-all duration-300">
                    <div className="flex flex-col items-center text-center gap-2 sm:gap-3">
                      <Shield className="w-6 h-6 sm:w-8 sm:h-8 text-blue-400" />
                      <span className="text-sm sm:text-base text-gray-200">Надежность</span>
                    </div>
                  </div>
                </div>
              </div>
